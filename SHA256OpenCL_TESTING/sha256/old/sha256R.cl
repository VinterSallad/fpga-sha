...
#define EXPANDARRAY \
    words[0] = M32(words[0], words[14], words[9], words[1]);   \
    words[1] = M32(words[1], words[15], words[10], words[2]);  \
    words[2] = M32(words[2], words[0], words[11], words[3]);   \
    words[3] = M32(words[3], words[1], words[12], words[4]);   \
    words[4] = M32(words[4], words[2], words[13], words[5]);   \
    words[5] = M32(words[5], words[3], words[14], words[6]);   \
    words[6] = M32(words[6], words[4], words[15], words[7]);   \
    words[7] = M32(words[7], words[5], words[0], words[8]);    \
    words[8] = M32(words[8], words[6], words[1], words[9]);    \
    words[9] = M32(words[9], words[7], words[2], words[10]);   \
    words[10] = M32(words[10], words[8], words[3], words[11]);  \
    words[11] = M32(words[11], words[9], words[4], words[12]);  \
    words[12] = M32(words[12], words[10], words[5], words[13]); \
    words[13] = M32(words[13], words[11], words[6], words[14]); \
    words[14] = M32(words[14], words[12], words[7], words[15]); \
    words[15] = M32(words[15], words[13], words[8], words[0]);

uint32_t M32 (uint32_t w0, uint32_t w14, uint32_t w9, uint32_t w1)
{
    return sigma1_32(w14) + (w9) + sigma0_32(w1) + (w0);
}
...
uint8_t crypto_hashblocks_sha256(__local uint8_t *statebytes,
                                   __local uint8_t *in, uint8_t inlen, 
                                   __local uint32_t *words) {
    uint32_t state[8];
...
uint8_t crypto_hashblocks_sha256_g(__local uint8_t *statebytes,
                                   __global uint8_t *in, uint8_t inlen, 
                                   __local uint32_t *words) {
    uint32_t state[8];
    uint32_t workingVars[10];
    //Ladda working state variables
...
void sha_inc_finalize(__global uint8_t *out, __local uint8_t *state, __global uint8_t *in, uint64_t inlen, __local uint8_t *padded, __local uint32_t *words)
{

    uint64_t bytes = load_bigendian_64(state + 32) + inlen;  
    inlen = crypto_hashblocks_sha256_g(state, in, inlen, words);
    ...
    if (inlen < 56) {
        ...
        crypto_hashblocks_sha256(state, padded, 64, words);
    }else {
        ...
        crypto_hashblocks_sha256(state, padded, 128, words);
    }
...
__kernel void sha256(__global uint8_t *out, __global uint8_t *in, 
                    __global uint64_t *inlen, __local uint8_t *state, 
                    __local uint8_t *padded, __local uint32_t *words)
{
    sha_inc_init(state);
    sha_inc_finalize(out, state, in, *inlen, padded, words);	
}
