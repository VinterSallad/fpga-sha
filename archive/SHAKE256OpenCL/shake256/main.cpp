...
int main(int argc, char const *argv[])
{
    ...
    */
    ...
    /* Load the kernel */
    { cl_int status;
        kernel = clCreateKernel(program, "ROL", &status);
    ...
    cl_mem ROL_on_FPGA;
    { cl_int status;
        ROL_on_FPGA = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(uint64_t), NULL, &status);
        checkError(status, "Failed to allocate memory on device");
    }

    uint64_t ROL_on_CPU = 1;
    /* Copy Vector_on_CPU to Vector_on_FPGA */
    { cl_int status;
        status = clEnqueueWriteBuffer(queue, ROL_on_FPGA, CL_TRUE, 0,
                    sizeof(uint64_t), &ROL_on_CPU, 0, NULL, NULL);
	...
	int rotateSize = 8;
    {
        ...
        cl_int status;
        status = clSetKernelArg(kernel, 0, sizeof(cl_mem), &ROL_on_FPGA);
	status = clSetKernelArg(kernel, 1, sizeof(int), &rotateSize);
    ...
    { cl_int status;
        status = clEnqueueReadBuffer(queue, ROL_on_FPGA, CL_TRUE, 0,
                    sizeof(uint64_t), &ROL_on_CPU, 0, NULL, NULL);
        checkError(status, "Failed to transfer data from host to device"); }
